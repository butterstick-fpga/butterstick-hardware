...
# Basic UDP stream, but with added timeout to assist with less packet overhead.
# This module will wait on an incomming stream to create packet payloads of 1280 bytes
# This really helps out on the software side to avoid flooding the netwark stack
...
class LiteEthTTYTX(Module):
    def __init__(self, ip_address, udp_port, fifo_depth=None):
        ...
        if fifo_depth is None:
            ...
            timeout = Signal(max=int(100e6))
            timeout_reset = Signal()
            self.sync += \
                If(timeout_reset,
                    timeout.eq(0)
                ).Elif(timeout <= int(5e6),
